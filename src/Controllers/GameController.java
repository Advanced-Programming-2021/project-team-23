...
public class GameController {
    ...
    public void phaseController(int newPhaseNumber){
        Duel.printPhaseName(newPhaseNumber);
        if(newPhaseNumber == 1){
            if(boards[currentPlayer].getDeckZone().size() == 0){
                ...
                if(boards[currentPlayer].canAnyCardBeChosenForThisBoardInDrawPhase() &&
                    ...
                    Duel.printSuccessfulAddingCardInDrawPhase(card);
                }
                duel.printBoards();
            }
        }
    ...
    public void printWinnerOfDuel(){
        duel.playEndGameSound();
        int winner = winners[currentRound - 1];
        IMenu.showSuccess(getUser(winner).getUsername() + " won the game and the score is: " +
    ...
    public void printWinnerOfMatch(){
        IMenu.showSuccess(getUser(winner).getUsername() + " won the whole match with score: " +
                getUser(0).getScore() + "-" + getUser(1).getScore());
    }
