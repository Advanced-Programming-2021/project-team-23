...
public class Card {
    ...
    public transient static ImageView backImageView;

    static {
        try {
            backImageView = new ImageView(new Image(new FileInputStream(Duel.assetsAddress + "\\Cards\\Monsters\\Unknown.jpg")));
    ...
    public Card(String name){
        ...
        if(this.isMonster()) cardFileName += "Monsters\\";
        else cardFileName += "SpellTrap\\";
        cardFileName += getCardFileName(name) + ".jpg";
        try {
            imageView = new ImageView(new Image(new FileInputStream(cardFileName)));
        } catch (FileNotFoundException e) {
            e.printStackTrace();
            imageView = new ImageView(backImageView.getImage());
        }
        currentImageView = new ImageView[2];
    }

    public String getCardFileName(String name){
        StringBuilder fileName = new StringBuilder();
        if(name.startsWith("Terratiger")) return "Terratiger";
        name = name.replaceAll("'|-", "");
        String[] strings = name.split(" ");
        for(String string: strings){
            StringBuilder stringBuilder = new StringBuilder(string);
            stringBuilder.setCharAt(0, Character.toUpperCase(string.charAt(0)));
            fileName.append(stringBuilder);
        }
...
//        return null;
    ...
    public void setMode(String mode) {
        this.mode = mode;
        if(mode.contains("H")) {
            currentImageView[0].setImage(backImageView.getImage());
            currentImageView[1].setImage(backImageView.getImage());
        }
        else {
            currentImageView[0].setImage(imageView.getImage());
            currentImageView[1].setImage(imageView.getImage());
        }
    }
